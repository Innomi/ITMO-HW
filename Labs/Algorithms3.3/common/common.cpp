...
const unsigned int MAXLEN = 200002;
...
int main()
{
    ...
    unsigned int pos(0), maxLength(0);
    
    
    for (size_t i = 1; i < sufArray.size(); ++i)
    {
        if (((sufArray[i] < first && sufArray[i - 1] > first) || (sufArray[i] > first && sufArray[i - 1] < first)) && (lcpArray[i - 1] > maxLength))
        {
            maxLength = lcpArray[i - 1];
            pos = sufArray[i];
        }
    }
