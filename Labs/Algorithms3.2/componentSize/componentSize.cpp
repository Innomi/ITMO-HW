...
void zig(node* x) //NAS POSADYAT!!!
{
    ...
    if (p->l != nullptr)
    {
        p->subtreeSize += p->l->subtreeSize;
    }
    if (p->r != nullptr)
    {
        p->subtreeSize += p->r->subtreeSize;
    }
    x->subtreeSize = x->v;
    if (x->l != nullptr)
    {
        x->subtreeSize += x->l->subtreeSize;
    }
    if (x->r != nullptr)
    {
        x->subtreeSize += x->r->subtreeSize;
    }
}
...
void zig_zig(node* x) //NAS TOCHNO POSADYAT!!!
{
    ...
    if (g->l != nullptr)
    {
        g->subtreeSize += g->l->subtreeSize;
    }
    if (g->r != nullptr)
    {
        g->subtreeSize += g->r->subtreeSize;
    }
    p->subtreeSize = p->v;
    if (p->l != nullptr)
    {
...
unsigned int size(node* u)
{
    while (u->p != nullptr)
    {
        u = u->p;
    }
    return u->subtreeSize;
}
...
int main()
{
    ...
    for (size_t i = 0; i < m; ++i)
    {
        ...
        else
        {
            ...
            {
                cout << size(edges[evaluateEdge(x, x)]) << endl;
            }
        }
    }
    return 0;
